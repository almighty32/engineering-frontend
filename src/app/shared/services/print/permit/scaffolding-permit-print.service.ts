...
export class ScaffoldingPermitPrintService {
  constructor(private dateService: DateService) {}

  async print(data: IScaffoldingPermitModel) {
    const element = data;
    try {
      const secondPdfBytes = await fetch('/assets/pdfs/scaffolding_permit_form.pdf').then(res =>
        res.arrayBuffer()
      );

      const pdfDoc = await PDFDocument.load(secondPdfBytes);
      pdfDoc.setTitle('Scaffolding Permit');
      ...
      if (element.isAccesoryBuildingOrStructure) {
  ...
  async printList() {
    ...
    // ELEMENT_DATA.forEach((element: IScaffoldingPermitModel) => {
    //   compiledData.push([
