...
export class FencingPermitPrintService {
  constructor(private dateService: DateService) {}

  async print(data: IFencingPermitModel) {
    const element = data;
    try {
      const secondPdfBytes = await fetch('/assets/pdfs/fencing_permit_form.pdf').then(res =>
        res.arrayBuffer()
      );

      const pdfDoc = await PDFDocument.load(secondPdfBytes);
      pdfDoc.setTitle('Fencing Permit');
      ...
      form.getTextField('FP_NO').setText(cutAreaNo);
      ...
      if (element.isIndigenousMaterials) {
        form.getCheckBox('INDIGENOUS_MATERIAL').check();
      }
      if (element.isReinforceConcrete) {
        form.getCheckBox('RC').check();
      }
      if (element.isRCAndConcHollowBlocks) {
        form.getCheckBox('RC_AND_CONC').check();
      }
      if (element.isRCAndBricks) {
        form.getCheckBox('RC_AND_BRICKS').check();
      }
      if (element.isRCAndInterlinkOrCycloneWire) {
        form.getCheckBox('RC_AND_INTERLINK').check();
      }
      if (element.isRCSteelMatting) {
        form.getCheckBox('RC_STEEL').check();
      }
      if (element.isRCBarbedWire) {
        form.getCheckBox('RC_BARBED_WIRE').check();
      }

      if (element.othersFencingType != '') {
        form.getCheckBox('OTHER_FENCE').check();
      }
  ...
  async printList() {
    ...
    //   ELEMENT_DATA.forEach((element: FencingPermitModel) => {
    //     compiledData.push([
