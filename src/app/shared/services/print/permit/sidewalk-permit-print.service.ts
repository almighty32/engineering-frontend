...
export class SidewalkPermitPrintService {
  constructor(private dateService: DateService) {}

  async print(data: ISidewalkConstructionPermitModel) {
    const element = data;
    try {
      const secondPdfBytes = await fetch('/assets/pdfs/sidewalk_construction_permit_form.pdf').then(
        res => res.arrayBuffer()
      );

      const pdfDoc = await PDFDocument.load(secondPdfBytes);
      pdfDoc.setTitle('Sidewalk Permit');
  ...
  async printList() {
    ...
    // ELEMENT_DATA.forEach((element: ISidewalkConstructionPermitModel) => {
    //   compiledData.push([
