...
export class GeneratorUninterruptedPowerSupplyPrintService {
  fileName = 'coo_generators_or_ups';
  constructor(private dateService: DateService) {}

  async print(data: IGeneratorOrUPSModelModel) {
    const element = data;
    try {
      ...
      const {
        ...
        kvaWatts,
        descOrManuf,
        incomingOrOutgoingVoltage,
        noOfPhases,
        noAndSizeOfIncomingCables,
        noAndSizeOfOutgoingCables,
      ...
      form.getTextField('V_LICENSED_DATE').setText(`${_vDateIssued}`);
      ...
      form.getTextField('KVA_WATTS').setText(`${kvaWatts}`);
      form.getTextField('DESCRIPTION_OR_MANUFACTURER').setText(`${descOrManuf.toUpperCase()}`);
      form.getTextField('INCOMING_OUTGOING_VOLTAGE').setText(`${incomingOrOutgoingVoltage}`);
      form.getTextField('INCOMING_OR_OUTGOING_VOLTAGE').setText(`${incomingOrOutgoingVoltage}`);
      form.getTextField('NO_OF_PHASES').setText(`${noOfPhases}`);
      form.getTextField('SIZE_OR_NO_OF_INCOMING_CABLES').setText(`${noAndSizeOfIncomingCables}`);
      form.getTextField('NO_SIZE_OF_INCOMING_CABLES').setText(`${noAndSizeOfIncomingCables}`);
      form.getTextField('NO_AND_SIZE_OF_INCOMING_CABLES').setText(`${noAndSizeOfIncomingCables}`);
      form.getTextField('NO_SIZE_OF_OUTGOING_CABLES').setText(`${noAndSizeOfOutgoingCables}`);
      form.getTextField('NO_AND_SIZE_OF_OUTGOING_CABLES').setText(`${noAndSizeOfOutgoingCables}`);
      form.getTextField('NO_SIZE_OF_GROUNDING_WIRES').setText(`${noAndSizeOfGroundingWires}`);
      ...
      const _inspectionOrDateIssued = this.dateService.formatDate(inspectionOrDateIssued);
      form.getTextField('INSPECTION_OR_DATE_ISSUED').setText(`${_inspectionOrDateIssued}`);
